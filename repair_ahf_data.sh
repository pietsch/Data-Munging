...
## date: 2013-01-16
## dependencies: (g)awk, R with package xslx (requires Java)
...
//	 {
	     if (46 != NF) {
		 printf(\"Inconsistent number of columns (%d instead of 46) in line %d of file %s. Stopping check at this point.\n\", NF, NR, \"$infile\");
                 exit 1
	     };
         }" $1
}
...
write.csv2(myDataFrame, file=\"excel_$1\", na=\"\", fileEncoding=\"CP1252\")  ## write CSV for Excel for Windoze
library(\"xlsx\")
write.xlsx(myDataFrame, file=\"$1.xlsx\")
" | R --vanilla --slave
}

for infile in HB*_BI.[Cc]sv
do
    ...
    echo "File $infile should be OK now. Let's produce some real CSV files from it."
